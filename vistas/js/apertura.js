...
$("#email").change(function () {
  if ($(this).val() != "") {
    ...
    formData.append('correo', $(this).val());
      ...
      $.ajax({
          ...
          success: function (respuesta) {
            ...
            if(respuesta=='1'){
              $('#btnOpenCount').attr('disabled', true);
              $("#email").val("");
              $("#verificaremail").val("");
              swal({
                title: "Error",
                text: "El correo ya tiene una cuenta vinculada",
                type: "error",
                confirmButtonText: "¡Cerrar!"
              });
            }else{
              $('#btnOpenCount').attr('disabled', false);
...
//     var frontal = $("#frontal").val();
//     if(frontal == null){
//       alert('No ha seleccionado una imagen');
//     }else{
//       var extension = frontal.replace(/^.*\./, ''); 
//       if (extension == frontal){
//         extension = '';
//       }else{
...
$(".guardarRegistro").click(function () {
  var filename = $("#frontal").val();
  var tracera = $("#trasera").val();
  var comprobante_domici = $("#comprobante").val();
  if(filename== "" ||tracera==""  || comprobante_domici==""){
    alert("Debes seleccionar un documento de identificación y subir archivos");
  }
  ...
  if (nombre == "" || apellidos == "" || genero == "" || datepicker2 == "" || email == "" || pais == "" || cuenta == "" || telefono == "" ||
                ...
                $.ajax({
                  ...
                  success: function (response) {
                    if (response != 0) {
                      ...
                      datos.append("datepicker", datepicker2);
